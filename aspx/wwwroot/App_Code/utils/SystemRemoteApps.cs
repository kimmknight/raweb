...
namespace RAWebServer.Management {
  public class SystemRemoteApps {
    ...
    public class SystemRemoteApp {
      ...
      public CommandLineMode CommandLineOption { get; set; }
      public bool IncludeInWorkspace { get; set; }
      public FileTypeAssociationCollection FileTypeAssociations { get; set; }
      ...
      public SystemRemoteApp(string name, string fullName, string path, string vPath, string iconPath, int iconIndex, string commandLine, CommandLineMode commandLineOption, bool includeInWorkspace, FileTypeAssociationCollection fileTypeAssociations, RawSecurityDescriptor securityDescriptor) {
        Name = name;
        FullName = fullName;
        Path = path;
        VPath = vPath;
        ...
        FileTypeAssociations = fileTypeAssociations ?? new FileTypeAssociationCollection();
      ...
      public void WriteToRegistry() {
        RequireElevatedPrivileges(); // TODO: execute this in a separate service with administrative privileges
        EnsureRegistryPathExists();

        using (var appsKey = Registry.LocalMachine.OpenSubKey(s_applicationsRegistryPath, true)) {
          using (var appKey = appsKey.CreateSubKey(Name)) {
            appKey.SetValue("FriendlyName", FullName);
            ...
            appKey.SetValue("CommandLineSetting", CommandLine);
            appKey.SetValue("CommandLineSettingOption", CommandLineOption);
      ...
      public void DeleteFromRegistry() {
        ...
        using (var appsKey = Registry.LocalMachine.OpenSubKey(s_applicationsRegistryPath, true)) {
          appsKey.DeleteSubKeyTree(Name, false);
        }
      }
    }
      ...
      using (var appsKey = Registry.LocalMachine.OpenSubKey(s_applicationsRegistryPath)) {
        using (var appKey = appsKey.OpenSubKey(appName)) {
          ...
          var app = new SystemRemoteApp(
            name: appName,
            fullName: Convert.ToString(appKey.GetValue("FriendlyName", appName)),
            path: Convert.ToString(appKey.GetValue("Path", "")),
            vPath: Convert.ToString(appKey.GetValue("VirtualPath", "")),
            iconPath: Convert.ToString(appKey.GetValue("IconPath", "")),
    ...
    private InstalledApp TranslateShortcutToInstalledApp(string shortcutFilePath, string programsPath) {
      ...
      var targetArguments = (string)shortcut.Arguments;
    ...
    public RegistryUserProfileCollection GetAllUserProfiles() {
      ...
      foreach (var sid in Registry.LocalMachine.OpenSubKey(registryProfileListPath).GetSubKeyNames()) {
        using (var profileKey = Registry.LocalMachine.OpenSubKey(registryProfileListPath + @"\" + sid)) {
          ...
          var exists = System.IO.Directory.Exists(profilePath);
          if (!exists) {
            continue;
          }
