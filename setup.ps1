...
function Import-GuacdImage([Parameter(Mandatory=$true)][string]$tag, [Parameter(Mandatory=$true)][string]$digest) {
    ...
    try {
        $tmp_dir = "guacd-import-tmp-$tag"
        if (Test-Path -Path $tmp_dir) {
            Remove-Item -Path $tmp_dir -Recurse -Force
        }

        Write-Host "Pulling guacamole/guacd image with tag '$tag' from Docker Hub..."

        # get bearer token
        $bearer = Invoke-RestMethod -Uri "https://auth.docker.io/token?service=registry.docker.io&scope=repository:guacamole/guacd:pull" | Select-Object -ExpandProperty token

        # pull the guacd image using the bearer token
        $manifestUrl = "https://registry-1.docker.io/v2/guacamole/guacd/manifests/$digest"
        $layers = Invoke-RestMethod -Uri $manifestUrl -Headers @{ Authorization = "Bearer $bearer" }

        # require schema version 2
        if ($layers.schemaVersion -ne 2) {
            throw "Unsupported schema version: $($layers.schemaVersion). Only schema version 2 is supported."
        }
        
        # require image schema 1
        $isDockerImageV1 = $layers.config.mediaType -eq "application/vnd.docker.container.image.v1+json"
        $isOCIImageV1 = $layers.mediaType -eq "application/vnd.oci.image.manifest.v1+json"
        if (-not $isDockerImageV1 -and -not $isOCIImageV1) {
            Write-Output $layers | ConvertTo-Json -Depth 5
            $mediaType = if ($layers.config.mediaType) { $layers.config.mediaType } else { $layers.mediaType }
            throw "Unsupported image schema: $mediaType. Only application/vnd.docker.container.image.v1+json and application/vnd.oci.image.manifest.v1+json are supported."
        }

        # make a directory to store the layers
        $layerDir = Join-Path -Path $tmp_dir -ChildPath "layers"
        if (-not (Test-Path -Path $layerDir)) {
            New-Item -ItemType Directory -Path $layerDir | Out-Null
        }

        # download each layer
        foreach ($layer in $layers.layers) {
            if ($layer.mediaType -ne "application/vnd.docker.image.rootfs.diff.tar.gzip" -and $layer.mediaType -ne "application/vnd.oci.image.layer.v1.tar+gzip") {
                throw "Unsupported layer media type: $($layer.mediaType). Only application/vnd.docker.image.rootfs.diff.tar.gzip is supported."
            }

            if (-not $layer.digest) {
                throw "Layer is missing digest: $($layer | ConvertTo-Json -Depth 1)"
            }

            if (-not $layer.size) {
                throw "Layer is missing size: $($layer | ConvertTo-Json -Depth 1)"
            }

            $targzFile = "$layerDir/$($layer.digest.Replace("sha256:", '')).tar.gz"
            $ProgressPreference = 'SilentlyContinue'
            Invoke-RestMethod -Headers @{ Authorization = "Bearer $bearer" } -Uri "https://registry-1.docker.io/v2/guacamole/guacd/blobs/$($layer.digest)"  -OutFile $targzFile
        }

        Write-Host "Preparing layers..."

        # un-gzip each layer and extract the tar file
        $layerTars = @()
        foreach ($layer in $layers.layers) {
            $in = "$([System.IO.Path]::GetTempPath())$layerDir/$($layer.digest.Replace("sha256:", '')).tar.gz"
            $out = "$([System.IO.Path]::GetTempPath())$layerDir/$($layer.digest.Replace("sha256:", '')).tar"
            $inStream = [IO.File]::OpenRead($in)
        ...
        # unzip installer dependencies
        $installerDepsZip = Join-Path -Path $original_dir -ChildPath "installer-deps.zip"
        $installerDepsDir = Join-Path -Path $tmp_dir -ChildPath "installer-deps"
        if (-not (Test-Path -Path $installerDepsZip)) {
            throw "installer-deps.zip not found. Please make sure installer-deps.zip is in the current directory."
        }
        Expand-Archive -Path $installerDepsZip -DestinationPath $installerDepsDir -Force
        ...
        $etcDir = Join-Path -Path $tmp_dir -ChildPath "etc"
        New-Item -ItemType Directory -Force -Path $etcDir | Out-Null
        $configPath = Join-Path $etcDir "wsl-distribution.conf"
@"
[oobe]
command = cd /opt/guacamole
defaultName = guacd-$tag

[user]
default = guacd

[boot]
command = cd /opt/guacamole && /opt/guacamole/entrypoint.sh
...
"@ | Out-File $configPath -Encoding UTF8
        $currntDir = Get-Location
        $wslFileName = Split-Path -Path $wslFile -Leaf
        Set-Location -Path $tmp_dir
        try {
            & $tarExe --create --file=$wslFileName "etc"
        }
        finally {
            Set-Location -Path $currntDir
        }

        # concatenate all the tar files into one big tar file
        foreach ($tar in $layerTars) {
