...
A tiny exe with no installation logic of its own. It exists so a GitHub release or a preview build in `public.yaml` can offer a "just install this build" download without needing its own copy of the full [installer](../RAWeb.Server.Installer/README.md) rebuilt and re-signed each time.
