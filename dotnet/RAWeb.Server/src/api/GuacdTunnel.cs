...
internal static class GuacdTunnelEndpoint {
  private static readonly ConcurrentHashSet<string> s_activeConnectionIds = new();
  private static readonly Logger s_logger = new("guacd-tunnel");
  private static readonly AsyncDebouncer s_guacdStopDebouncer = new();
  private static readonly AsyncDebouncer s_guacdUninstallDebouncer = new();

  internal static void Map(WebApplication app) {
    app.MapGet("/guacd-tunnel", Handle);
  }

  private static async Task Handle(HttpContext context) {
    Console.WriteLine("Received request to GuacdTunnel handler.");
    if (!context.WebSockets.IsWebSocketRequest) {
      s_logger.WriteLogline("Received non-WebSocket request to GuacdTunnel handler.");
    ...
    if (!requestedProtocols.Contains("guacamole")) {
      s_logger.WriteLogline("WebSocket connection rejected due to missing 'guacamole' subprotocol.");
      context.Response.StatusCode = 400;
      await context.Response.WriteAsync("WebSocket subprotocol 'guacamole' required.");
      return;
    }

    s_logger.WriteLogline("Accepting WebSocket connection with 'guacamole' subprotocol.");
    ...
    if (userInfo == null) {
      s_logger.WriteLogline("WebSocket connection rejected due to unauthenticated user.");
    ...
    await ProcessWebSocket(ws, context);
  }
  ...
  private static async Task ProcessWebSocket(WebSocket ws, HttpContext context) {
    ...
    try {
      ...
      var userInfo = UserInformation.FromHttpRequestSafe(context.Request);
      if (userInfo == null) {
        // send unauthorized user error
        await sendToBrowser(GuacEncode("error", "You are not authenticated.", "401"));
        await disconnectBrowser();
        return;
      }
      s_logger.WriteLogline($"User '{userInfo.Username}' authenticated successfully.");

      // check that there is a resource available and that the user has permission to access it
      var resourcePath = (string?)context.Request.Query["rPath"];
      var resourceFrom = (string?)context.Request.Query["rFrom"];
      ...
      s_logger.WriteLogline($"User '{userInfo.Username}' is requesting resource at path '{resourcePath}' from '{resourceFrom}'.");
      ...
      try {
        ...
        if (resolvedResource is ResourceContentsResolver.FailedResourceResult failedResource) {
          s_logger.WriteLogline($"Failed to resolve resource for user '{userInfo.Username}': {failedResource.ErrorMessage}");
        ...
        s_logger.WriteLogline($"Resource resolved successfully for user '{userInfo.Username}'. Preparing RDP connection...");
        rdpContents = ((ResourceContentsResolver.ResolvedResourceResult)resolvedResource).RdpFileContents;
      }
      catch (Exception ex) {
      ...
      if (string.IsNullOrEmpty(fullAddress)) {
        s_logger.WriteLogline($"RDP file for user '{userInfo.Username}' is missing the full address property.");
      ...
      s_logger.WriteLogline($"Extracted connection details - Address: {fullAddress}, Port: {port}");
      ...
      if (gatewayUsageMethod == GatewayUsageMethod.UseIfNoDirectConnectionPossible) {
        ...
        try {
          ...
          if (completedTask == timeoutTask) {
            s_logger.WriteLogline($"Direct connection to {fullAddress}:{port} failed or timed out. Will use gateway.");
            shouldUseGateway = true;
          }
        }
        catch (Exception ex) {
          s_logger.WriteLogline($"Direct connection to {fullAddress}:{port} failed: {ex.Message}. Will use gateway.");
          shouldUseGateway = true;
        }
      }
      ...
      if (shouldUseGateway) {
        ...
        if (!string.IsNullOrEmpty(gatewayHostname) && gatewayHostname.Contains(":")) {
          ...
          gatewayPort = parts[1];
          s_logger.WriteLogline($"Gateway info - Method: {gatewayUsageMethod}, Hostname: {gatewayHostname}, Port: {gatewayPort}");
        }
      ...
      var shouldIgnoreGatewayCertificateErrors = allowIgnoreGatewayCertErrors && context.Request.Query["ignoreGatewayCertErrors"] == "true";
      ...
      var shouldIgnoreTermServCertificateErrors = context.Request.Query["ignoreCertErrors"] == "true";
      ...
      if (!shouldUseGateway) {
        try {
          ...
          s_logger.WriteLogline($"Failed to resolve hostname '{fullAddress}' to an IPv4 address: {ex.Message}");
        }
      }
      ...
      if (shouldUseGateway) {
        try {
          ...
          s_logger.WriteLogline($"Resolved gateway address to IPv4: {gatewayHostnameIPv4}");
        }
        catch (Exception ex) {
          await sendToBrowser(GuacEncode("error", "Failed to resolve gateway hostname to an IPv4 address: " + ex.Message, "10043"));
          s_logger.WriteLogline($"Failed to resolve gateway hostname '{gatewayHostname}' to an IPv4 address: {ex.Message}");
        }
      }
      ...
      s_logger.WriteLogline($"Initial display info received - Width: {displayWidth}, Height: {displayHeight}, DPI: {displayDpi}");
      ...
      s_logger.WriteLogline($"Timezone info received - Timezone: {timezone}");
      ...
      if (shouldUseGateway) {
        ...
        s_logger.WriteLogline($"Gateway credentials resolved - Hostname: {gatewayHostname}, Port: {gatewayPort}, Domain: {gatewayDomain}, Username: {gatewayUsername}");
      }
      ...
      try {
        ...
        if (guacdMethod == "external") {
          var externalAddress = PoliciesManager.RawPolicies["GuacdWebClient.Address"];
          if (string.IsNullOrEmpty(externalAddress)) {
            await sendToBrowser(GuacEncode("error", "Guacd method is set to external, but no address is specified.", "10045"));
            await disconnectBrowser();
            return;
          }
          guacdAddress = externalAddress;
        }
        ...
        // start the internal guacd
          ...
          try {
            ...
            if (!Guacd.IsRunning) {
              s_logger.WriteLogline($"Failed to start guacd: {ex}");
            ...
            s_logger.WriteLogline($"Exception occurred while starting guacd, but guacd is running: {ex}");
          }

          guacdAddress = Guacd.IpAddress + ":4822";
        }

        s_logger.WriteLogline($"Connecting to guacd at {guacdAddress} using method '{guacdMethod}'.");
        ...
        try {
          ...
          using (var stream = guacd.GetStream()) {
            ...
            IEnumerable<string?> connectArgs = argsInstruction.AcceptedParameterNames.Select(paramName => {
            ...
            var fromGuacd = Task.Run(async () => {
              ...
              s_logger.WriteLogline($"Guacd -> browser connection closed for user '{userInfo.Username}' and resource '{resourcePath}'.");
            });
          ...
          s_logger.WriteLogline($"SocketException: Unable to reach guacd server at {guacdHostname}:{guacdPort} - {ex.Message}");
          if (ex.SocketErrorCode == SocketError.HostNotFound ||
              ex.SocketErrorCode == SocketError.NoData ||
              ex.SocketErrorCode == SocketError.HostUnreachable ||
              ex.SocketErrorCode == SocketError.HostDown) {
          ...
          s_logger.WriteLogline($"An unexpected error occurred when attempting to connect to the guacd server (SocketException): {ex}");
          await disconnectBrowser();
          return;
        }
      }
        ...
        s_logger.WriteLogline("An error occurred during the remote desktop session: " + ex.Message);
        await sendToBrowser(GuacEncode("error", "An unexpected error occurred during the remote desktop session.", "10034"));
      }
      finally {
        s_logger.WriteLogline($"Remote desktop session ended for user '{userInfo.Username}' and resource '{resourcePath}'.");
  ...
  /// </summary>
  private static void ScheduleDelayedCleanup() {
    ...
    if (s_activeConnectionIds.Count == 0) {
      s_guacdStopDebouncer.DebounceAsync(FIVE_MINUTES_AS_MS, () => {
        ...
        s_logger.WriteLogline("Error while attempting to stop guacd after all connections closed: " + ex);
      });
    }
    ...
    if (s_activeConnectionIds.Count == 0) {
      s_guacdUninstallDebouncer.DebounceAsync(FIFTEEN_MINUTES_AS_MS, () => {
        ...
        s_logger.WriteLogline("Error while attempting to uninstall guacd after all connections closed: " + ex);
      });
    }
  }
