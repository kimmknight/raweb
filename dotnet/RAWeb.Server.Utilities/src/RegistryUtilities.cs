...
public class RegistryReader {
    ...
    public static string ConstructRdpFileFromRegistry(string keyName, bool? isDesktop = false) {
        ...
        // read the additional properties from the policies
        ...
        if (isDesktop == true) {
            if (!supportsCentralizedPublishing) {
                throw new InvalidOperationException("Centralized publishing is not enabled on this server.");
            }

            // generate the RDP file contents for the desktop
