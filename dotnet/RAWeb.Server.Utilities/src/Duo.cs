...
public sealed class DuoAuth(string clientId, string apiSecret, string apiHostname, string redirectPath) {
  ...
  public void DoHealthCheck() {
    var clientAssertion = CreateClientAssertionJwt($"https://{ApiHostname}/oauth/v1/health_check");
  ...
  /// MFA authorization. This is used in the state variable in the request to Duo, which
  ...
  public string GetRequestAuthorizationEndpoint(string domain, string username, string returnUrl) {
    ...
    var requestJwt = Jwt.Create(new Dictionary<string, object> {
      ...
      ["state"] = state,
      ["duo_uname"] = username,
      ["aud"] = $"https://{ApiHostname}/oauth/v1/authorize",
      ["dest_app_name"] = $"RAWeb on {s_aliasResolver.Resolve(Environment.MachineName)}",
      ["dest_app_id"] = AppId.ToGuid().ToString(),
      ["iss"] = ClientId
    });
  ...
  public VerifyResponseResult VerifyResponse(string code, string state) {
    ...
    var clientAssertionJwt = CreateClientAssertionJwt($"https://{ApiHostname}/oauth/v1/token");
