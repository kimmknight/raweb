...
internal sealed class Hs512Jwt(string secret) {
  ...
  public string Create(IDictionary<string, object> claims) {
    ...
    var payload = Base64UrlEncode(Encoding.UTF8.GetBytes(payloadObj.ToJsonString()));
  ...
  /// <returns>A JsonObject containing the claims from the token payload if the signature is valid.</returns>
  ...
  public JsonObject VerifyAndDecode(string token) {
    ...
    var decoded = JsonNode.Parse(Encoding.UTF8.GetString(Base64UrlDecode(parts[1])))?.AsObject();
    if (decoded is null) {
      throw new InvalidOperationException("ID token payload is not a valid JSON object");
    }
    return decoded;
  }
