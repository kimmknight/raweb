...
- Only include screenshots that 30% quality WEBP format to reduce file size while maintaining readability.
- Use the `screenshot` CSS class on all screenshots to ensure consistent styling.
- Specify alt text for accessibility.
- Always specify a width. Never specify a height. RAWeb's documentation site will automatically calulate the appropriate height to maintain the aspect ratio of the image and prevent content layout shifts as the image loads.
