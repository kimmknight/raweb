<script setup lang="ts">
  ...
  const {
    ...
    /* if provided, indicates that this dialog is being used as one step in a multi-file import */
    bulkWizard?: {
      currentIndex: number;
      totalCount: number;
    };
  ...
  function clearDialogData() {
    ...
    if (contentElem.value) {
      contentElem.value.closest('.content-dialog-body')?.scrollTo(0, 0);
    }
  }

  var isContentVisible = ref(true);
  async function fadeOutDialogContent() {
    if (contentElem.value) {
      isContentVisible.value = false;
      await fadeOut(contentElem.value);
    }
  }
  async function revertToInitialData(fadeOut = true) {
    if (fadeOut) {
      await fadeOutDialogContent();
    }

    clearDialogData();
    openDate.value = Date.now();

    if (contentElem.value) {
      await nextTick();
      isContentVisible.value = true;
      await entranceIn(contentElem.value);
    }
  }
  ...
  async function attemptSave(close: () => void) {
    ...
    await fetch(`${iisBase}api/management/resources/registered`, {
      ...
      .then(() => {
        ...
        const event = new PreventableEvent({ next, revertToInitialData, fadeOutDialogContent });
  ...
  function skip(close: () => void) {
    ...
    const event = new PreventableEvent({ next, revertToInitialData, fadeOutDialogContent });
    emit('afterSkip', event);

    if (!event.defaultPrevented) {
      console.error(
        'afterSkip event must be be prevented and fully handled by the parent component to ensure proper flow. Hiding content instead.'
      );

      fadeOut(contentElem.value);
...
<template>
  ...
  >
            ...
            <div class="split">
              ...
              <Field no-label-focus class="group">
                ...
                <div class="stack">
                  <img
                    :src="
                      uploadedDarkIconUrl || iconPath('dark') || iconPath('light') || iconPath('dark', true)
                    "
                    alt=""
                    height="36"
                    :style="`
    ...
    <template #footer="{ close }">
      <Button
        @click="attemptSave(close)"
        :loading="saving"
        :disabled="!hasRequiredFields || !openDate || !isContentVisible"
      >
        {{ t('dialog.ok') }}
      </Button>
      <Button
