...
import DocumentationSearchResults from './DocumentationSearchResults.vue';
import i18n, { i18nextPromise } from './i18n.ts';
import { useCoreDataStore } from './stores/index.mjs';

/**
 * Creates a Vue application for the documentation site.
 *
 * This function can be used both for client-side rendering and server-side rendering (SSR).
 *
 * For SSR, pass `createSSRApp` as the `createVueApp` option. To render a specific route,
 * pass the desired route path as the `initialPath` option.
 *
 * @param {object} options
 * @param {boolean?} [options.ssr=false]
 * @param {string?} [options.initialPath]
 */
export async function createDocsApp({ ssr = false, initialPath } = {}) {
  const createVueApp = ssr === true ? createSSRApp : createApp;
  const app = i18n(createVueApp(Documentation));
...
// if running in the browser, create and mount the app
