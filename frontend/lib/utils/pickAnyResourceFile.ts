 ...
 */
export async function pickAnyResourceFile(allowMultiple?: false, acceptOverride?: string): Promise<File>;
export async function pickAnyResourceFile(allowMultiple: true, acceptOverride?: string): Promise<FileList>;
export async function pickAnyResourceFile(
  allowMultiple = false,
  ...
  return new Promise<File | FileList>((resolve, reject) => {
    ...
    input.type = 'file';
    input.accept = acceptOverride ?? '.rdp,.resource,.tsresource' + (allowMultiple ? ',.tsresourcebundle' : '');
